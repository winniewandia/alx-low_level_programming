 ...
 * @filename: File whose contents are copied
 * @fd: Return value for open filename
 */

void print_error_and_exit(int error_code, const char *message, const char *file_name, int fd)
{
...
int main(int argc, char **argv)
{
	...
	while ((bytes_read = read(fd_from, buffer, BUFFER_SIZE)) > 0)
	{
		...
		if (bytes_written < bytes_read)
			print_error_and_exit(99, "Error: Short write to %s\n", file_to, fd_from);
	}
	if (bytes_read == -1)
		print_error_and_exit(98, "Error: Can't read from file %s\n", file_from, fd_to);
	if (close(fd_from) == -1)
	...
	if (stat(file_to, &file_stat) == -1)
		print_error_and_exit(99, "Error: Can't get file status of %s\n", file_to, -1);
	if ((file_stat.st_mode & S_IFMT) == S_IFREG && file_stat.st_nlink == 1)
	{
		if (chmod(file_to, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH) == -1)
			print_error_and_exit(99, "Error: Can't change file permissions of %s\n", file_to, -1);
	}
	return (0);
}
